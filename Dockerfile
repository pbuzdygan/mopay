...
RUN cd frontend && npm run build || (echo "⚠️ Frontend build failed, placeholder" && mkdir -p dist && \
    printf '<!doctype html><html><head><meta charset=\"utf-8\"><title>Mopay</title></head><body><h1>Frontend fail to build. Execute to see details \n docker compose build --no-cache --progress=plain</h1></body></html>' > dist/index.html)
